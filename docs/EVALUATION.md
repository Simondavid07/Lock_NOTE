...
| Innovation & Meaningful Differentiation | [Comparison guide](COMPARISON.md), proof-based delivery receipts, optional encrypted recipient replies, private one-use encrypted-file leases, Guardian Wipe threshold revocation, seal fingerprints, dead switches, and encrypted-note lifecycle controls. |
...
| Recover sender control | Provides owner preview, proof-based envelope-open acknowledgement, optional encrypted recipient replies, and owner withdrawal. | API reference and proof/reply lifecycle tests. |
...
| Encrypted recipient replies | The sender can opt in to short voluntary replies. A raw capability stays inside authenticated ciphertext, replies use a separate AES-GCM AAD domain, and only the owner capability can retrieve opaque reply envelopes for local decrypt. |
...
| Cryptography | Web Crypto API | AES-256-GCM encryption, authenticated v2 proof/reply envelopes with domain-separated AAD, browser Shamir-style Guardian Wipe splitting, fixed PBKDF2/HKDF policy, and integrity checks. |
| API | Express 5, Zod, Helmet, rate limits | Strict crypto-schema validation, lifecycle state transitions, owner/guardian/reply verifier checks, one-use private file leases, proof acknowledgements, controlled deletion, request IDs, safe timing, and structured redacted operational logs. |
| Persistence | Supabase Postgres, private Storage, Auth, Realtime | Ciphertext records, hash-only proof/guardian/reply/lease state, private encrypted file objects, opaque reply envelopes, provider-backed GitHub sign-in, ephemeral collaboration signals, and owner-only profile/contact metadata. |
...
| Unit and integration coverage | `npm run test` currently passes **95 tests**: proof-envelope and fixed-KDF behavior, Guardian share quorum/tamper cases, strict schemas, safe metadata, one-use/expiring private file leases, burn lifecycle, receipt replay, encrypted reply capability/owner/dead-switch regressions, and API behavior. |
...
| Live lifecycle test | `API_URL=https://lock-note-sigma.vercel.app npm run test:live` exercises production health, proof-enabled create, safe metadata, preview, burn, one verified receipt acknowledgement, encrypted reply capability/owner cleanup, private one-use file delivery, Guardian Wipe, drafts, and owner remote wipe. |
...
7. Demonstrate a short optional encrypted reply after recipient-side decrypt, then show the owner locally opening it; state explicitly that it is voluntary and not identity authentication.
8. Demonstrate a private encrypted file: explain the one-use lease and that no Storage path reaches the recipient.
9. Demonstrate Guardian Wipe with a 2-of-3 quorum and state explicitly that guardians revoke but cannot decrypt.
10. Sign in with GitHub and show the protected profile, optional account bio, private contact shortcuts, and browser-local capability vault; explain the RLS and zero-knowledge boundary.
11. Explain that collaboration occurs before sealing and is intentionally documented as a different trust boundary.
12. Show a keyboard skip-link/command-palette pass, then run or show the passing live smoke command.
...
| [`sql/007_encrypted_recipient_replies.sql`](sql/007_encrypted_recipient_replies.sql) | Opt-in reply verifier, private opaque-reply table, server-role-only locked insertion function, and cascading cleanup. |
