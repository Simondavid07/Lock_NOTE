...
| Encrypted recipient replies | Sender may allow a recipient to send a short voluntary confirmation or follow-up after local decrypt. | The reply capability is inside authenticated ciphertext; reply plaintext is AES-GCM encrypted in the browser, stored as an opaque envelope, and returned only with the owner capability. |
...
| Lifecycle policy controls | Set burn-after-read, a time expiry, a passphrase gate, a dead-switch policy, Guardian Wipe quorum, or optional encrypted replies before sealing. | Compose page. |
...
5. Use the sender vault to preview, inspect a verified receipt, inspect optional encrypted replies, or withdraw the note while it remains active.
...
7. If encrypted replies were enabled, the recipient may voluntarily send a short reply. The browser encrypts it under a reply-specific AES-GCM authenticated-data domain before upload; this is not an authenticated identity system, so anyone holding the link may be able to reply or read replies.
8. For a file, the browser redeems the short-lived private lease once; it never receives a Storage object path.
9. If Guardian Wipe is enabled, trustees combine the threshold number of original cards at `/guardian-wipe` to withdraw future server delivery only.
...
Use this guide with [DEMO.md](DEMO.md) and [EVALUATION.md](EVALUATION.md). The recommended live sequence is: show the keyboard skip link/command palette, sign in with GitHub, show the profile/vault boundary, seal a short test note with optional encrypted replies, point out the fragment and fingerprint, open as a recipient, show the verified receipt and voluntary encrypted reply, demonstrate one private encrypted-file lease, and finally show a 2-of-3 Guardian Wipe with synthetic cards.
